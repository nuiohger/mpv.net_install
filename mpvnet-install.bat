...
:: Get mpvnet.exe location
set mpv_path=%~dp0mpvnet.exe
if not exist "%mpv_path%" call :die "mpvnet.exe not found"

:: Get mpv-document.ico location
set icon_path=%~dp0mpvnet.exe
if not exist "%icon_path%" call :die "mpv-document.ico not found"

:: Register mpvnet.exe under the "App Paths" key, so it can be found by
:: ShellExecute, the run command, the start menu, etc.
set app_paths_key=HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\mpvnet.exe
call :reg add "%app_paths_key%" /d "%mpv_path%" /f
call :reg add "%app_paths_key%" /v "UseUrl" /t REG_DWORD /d 1 /f

:: Register mpvnet.exe under the "Applications" key to add some default verbs for
:: when mpv is used from the "Open with" menu
set classes_root_key=HKLM\SOFTWARE\Classes
set app_key=%classes_root_key%\Applications\mpvnet.exe
...
call :reg add "%classes_root_key%\SystemFileAssociations\video\OpenWithList\mpvnet.exe" /d "" /f
call :reg add "%classes_root_key%\SystemFileAssociations\audio\OpenWithList\mpvnet.exe" /d "" /f
...
:add_verbs
	...
	:: Add "play" verb
	call :reg add "%key%\shell\play" /d "mpv.netで開く" /f
	call :reg add "%key%\shell\play\command" /d "\"%mpv_path%\" %mpv_args% \"%%%%L" /f

	:: add icon
	call :reg add "%key%\shell\play" /v "Icon" /t REG_SZ /d "%icon_path%" /f

	goto :EOF

:add_contextmenu
	set prog_id=%~1
	set prog_id_key=%classes_root_key%\Directory\shell\%prog_id%
	
	call :reg add "%prog_id_key%\command" /d "\"%mpv_path%\" %mpv_args% \"%%%%L" /f
	call :reg add "%prog_id_key%" /v "Icon" /t REG_SZ /d "%icon_path%" /f
	call :reg add "%prog_id_key%" /d "mpv.netで開く" /f
	goto :EOF
