...
reg delete "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\mpvnet.exe" /f >nul

:: Delete HKCR subkeys
set classes_root_key=HKLM\SOFTWARE\Classes
reg delete "%classes_root_key%\Applications\mpvnet.exe" /f >nul
reg delete "%classes_root_key%\SystemFileAssociations\video\OpenWithList\mpvnet.exe" /f >nul
reg delete "%classes_root_key%\SystemFileAssociations\audio\OpenWithList\mpvnet.exe" /f >nul
...
:: Delete context menu
reg delete "%classes_root_key%\Directory\shell\mpv" /f >nul
